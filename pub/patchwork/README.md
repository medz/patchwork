...
- `patchwork.lock` records source and applied-output safety state while
  Patchwork owns active edits or generated output. It is not the committed patch
  inventory.
...
Patchwork derives committed patch existence from safe
`patches/<pkg>@<version>.patch` filenames. Older patch files remain visible to
`status` and `doctor` as stale patch files until you carry them forward or remove
them.
