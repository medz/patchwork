...
import '../pub/override_value.dart';
...
final class DependencyOverrideState {
  ...
  factory DependencyOverrideState.read({
    ...
    final paths = <String>[];
    for (final path in overrideRootPaths) {
      if (!paths.any((existing) => p.equals(existing, path))) {
        paths.add(path);
      }
    }
    paths
      ..removeWhere((path) => p.equals(path, rootPath))
      ..sort()
      ..insert(0, rootPath);
    final roots = [
  ...
  bool hasActiveAppliedOverride(
    ...
    for (final root in _roots) {
      if (dependencyOverridePointsToPath(
        workspaceRootPath: root.path,
  ...
  DependencyOverrideConflict? userOwnedAppliedOverride(
    ...
    for (final root in _roots) {
      if (!p.equals(root.path, rootPath) &&
          dependencyOverridePointsToPath(
            workspaceRootPath: root.path,
            value: root.pubspecOverrideDependencyOverrides[marker.package],
            path: absoluteAppliedPath,
          )) {
  ...
  bool rootOverridePointsToPath({
    ...
    return dependencyOverridePointsToPath(
  ...
  Object? _rootRelativePathOverride(Object? value) {
    final path = dependencyOverridePath(value);
    if (path == null) {
      return value;
    }
...
bool _hasBlockingPathOverride({
  ...
  final existingPath = dependencyOverridePath(existing);
